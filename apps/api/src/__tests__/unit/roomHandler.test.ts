...
describe('game:start — private lobby coin charge', () => {
  ...
  it('charges ONLY the host 10 ⭐ on subsequent starts (hostPrepaid absent)', async () => {
    ...
    // inspect tx.user.updateMany rather than prisma.user.updateMany. Track
    // each call and assert exactly one host-targeted debit fires.
    ...
    // Exactly ONE debit call inside the transaction, targeting the host only
    // (NOT every player — they only pay in public rooms).
  ...
  it('aborts the start and emits game:start:failed when host has insufficient stars', async () => {
    ...
    // Atomic debit matches zero rows → host is broke. The callback throws,
    // which rolls the whole transaction back (no game row either).
    const txUpdateMany = vi.fn().mockResolvedValue({ count: 0 })
    const gameCreate = vi.fn()
    mockPrisma.$transaction.mockImplementation(async (fn: Function) =>
      fn({
    ...
    // Host debit was attempted (inside the tx). Game creation must NOT have
    // run — the throw short-circuits the callback before tx.game.create.
    expect(txUpdateMany).toHaveBeenCalledTimes(1)
